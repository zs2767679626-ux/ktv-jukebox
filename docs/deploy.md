...
4. 环境变量：
   ...
   - `NETEASE_COOKIE`：可选，网易云会员账号 Cookie，值形如 `MUSIC_U=xxx`。配置后 VIP 歌可正常播放；不配则 VIP 歌自动跳过（历史注明"版权受限"）。获取方式：浏览器登录 music.163.com → F12 → Application → Cookies → 复制 `MUSIC_U` 的整串值。建议用专门开通会员的公用小号。Cookie 会过期，失效后 VIP 歌恢复跳过，重新复制更新即可。
...
- [ ] 歌词滚动高亮；纯音乐正常显示
- [ ] VIP 歌有标签；未配 Cookie 时播不了自动跳下一首并注明，配了会员 Cookie 则正常播放
- [ ] 服务器重启：队列清空、历史保留
